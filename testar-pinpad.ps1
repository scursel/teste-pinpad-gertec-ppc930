...
if ($Chip) {
  ...
  if ($fr -and $fr.cmd -eq "SC03") {
    $s = Ascii $fr.payload
    if ($s.Length -gt 0 -and $s.Substring($s.Length - 1) -eq "1") { Write-Host "RESULTADO: CHIP PRESENTE (cartao inserido)" -ForegroundColor Green }
...
if ($Tarja) {
  ...
  if ($fr -and $fr.cmd -eq "MS06" -and $fr.payload.Count -gt 6) {
    if ($Completo) {
      ...
      Write-Host ("   EVENTO MS06 recebido (" + $fr.payload.Count + " bytes)") -ForegroundColor Green
    }
    $partes = @(); $cur = @()
